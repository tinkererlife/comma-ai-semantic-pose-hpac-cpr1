...
def apply_codes(source: Path, checkpoint: Path, output: Path) -> dict:
    ...
    if not np.array_equal(scales, coeff_scales):
...
def main() -> None:
    ...
    result = apply_codes(args.archive, args.checkpoint, args.out)
